...
class BERTEmbedding(torch.nn.Module):
    def __init__(self, vocab_size, embed_size, seq_len=64, dropout=0.1):
        ...
        self.segment(3, embed_size, padding_idx=0)
...
class EncoderLayer(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        self.layernorm_1 = torch.nn.LayerNorm(d_model)self.layernorm_2 = torch.nn.LayerNorm(d_model)
    ...
    def forward(self, embeddings, mask):
        interacted = self.dropout(self.multihead(query=embeddings, key=embeddings, value=embeddings, key_padding_mask=mask))
